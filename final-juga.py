...
# Fungsi untuk menyimpan data ke Firestore dengan jumlah objek terdeteksi
def save_to_firestore(timestamp, image_url, sleep_count):
    try:
        ...
        doc_ref.set({
            str(int(timestamp)): {
                ...
                'url_image': image_url,
                'sleep_count': sleep_count  # Menambahkan jumlah objek terdeteksi
            }
        }, merge=True)
        print(f"Data berhasil disimpan ke Firestore: {date_str}/{int(timestamp)}, Sleep Count: {sleep_count}")
    except Exception as e:
    ...
    for obj_id in list(tracked_objects.keys()):
        if obj_id in tracked_boxes:
            ...
            if not tracked_objects[obj_id]['message_sent'] and duration >= DETECTION_DURATION:
                ...
                if image_url:
                    # Simpan data ke Firestore dengan jumlah objek sleep
                    timestamp = time.time()
                    sleep_count = len(detections)  # Jumlah objek sleep terdeteksi
                    save_to_firestore(timestamp, image_url, sleep_count)
                    caption = f"Sleeping student detected (ID: {obj_id}, Count: {sleep_count})"
