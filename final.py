...
# Fungsi untuk menambahkan padding ke gambar 640x360 menjadi 640x640
...
# Fungsi untuk memotong gambar kembali ke ukuran asli
...
# Fungsi untuk menyesuaikan koordinat bounding box setelah cropping
...
# Fungsi untuk menghitung koordinat relatif
def calculate_relative_coords(box, image_width, image_height):
    x1, y1, x2, y2 = box
    return {
        'x1': x1 / image_width,
        'y1': y1 / image_height,
...
def save_to_firestore(timestamp, image_url, tracked_boxes, tracked_objects, image_width, image_height):
    try:
        # Pastikan date_str selalu string
        date_str = datetime.now().strftime("%Y-%m-%d")
        print("Generated date_str:", date_str, "Type:", type(date_str))
        if not date_str or not isinstance(date_str, str):
            print("Error: date_str is invalid or not a string:", date_str)
            return
        
        # Hitung jumlah mahasiswa yang tertidur
        num_sleeping = len(tracked_boxes)
        
        # Siapkan data untuk setiap mahasiswa yang tertidur
        sleeping_students = {}
        for obj_id, obj_data in tracked_boxes.items():
            box = obj_data['box']
            duration = time.time() - tracked_objects[obj_id]['start_time']
            absolute_coords = {'x1': box[0], 'y1': box[1], 'x2': box[2], 'y2': box[3]}
            relative_coords = calculate_relative_coords(box, image_width, image_height)
            sleeping_students[obj_id] = {
        ...
        data_entry = {
            ...
            'url_image': image_url,
            'num_sleeping_students': num_sleeping,
            'sleeping_students': sleeping_students
        }
        
        print("Data to save:", data_entry)
        print("Timestamp used:", timestamp, "Type:", type(timestamp))
        
        # Referensi dokumen dengan konversi eksplisit ke string untuk date_str
        doc_ref = db.collection('sleep').document(str(date_str))
        
        # Cek apakah dokumen sudah ada
        doc = doc_ref.get()
        if not doc.exists:
            # Jika dokumen belum ada, buat dengan array kosong
            doc_ref.set({'timestamps': []})
        
        # Tambahkan data ke array timestamps menggunakan arrayUnion
        doc_ref.update({
            'timestamps': firestore.ArrayUnion([data_entry])
        })
        
        print(f"Data berhasil disimpan ke Firestore: {date_str}/timestamps/{int(timestamp)}")
    except Exception as e:
...
next_id = 0
IMAGE_WIDTH, IMAGE_HEIGHT = 640, 360  # Dimensi gambar setelah resize
...
while True:
    ...
    # Sesuaikan koordinat bounding box untuk gambar asli (640x360)
    detections = adjust_bboxes(detections, padding)

    # Perbarui pelacakan
    tracked_boxes, next_id = update_tracks(tracked_boxes, detections)
    current_time = time.time()
    print("Current time:", current_time, "Type:", type(current_time))  # Debug waktu
    ...
    # Potong kembali gambar ke ukuran 640x360 untuk ditampilkan dan diunggah
    ...
    for obj_id in list(tracked_objects.keys()):
        if obj_id in tracked_boxes:
            ...
            if not tracked_objects[obj_id]['message_sent'] and duration >= DETECTION_DURATION:
                # Unggah gambar tanpa padding (640x360)
                image_url = upload_to_cloudinary(frame_for_upload)
                if image_url:
                    # Simpan data ke Firestore dengan informasi tambahan
                    timestamp = current_time
                    save_to_firestore(timestamp, image_url, tracked_boxes, tracked_objects, IMAGE_WIDTH, IMAGE_HEIGHT)
