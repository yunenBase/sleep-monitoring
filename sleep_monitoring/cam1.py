...
from database.backupp import save_to_firestore_detection
...
while True:
    ...
    frame_count += 1
    date_str = datetime.now().strftime("%Y-%m-%d")  # Hitung date_str sekali per iterasi
    ...
    # Update durasi untuk semua objek
    ...
    sleep_count = sum(1 for obj_id in tracked_objects.keys() if obj_id in tracked_boxes and tracked_objects[obj_id]['total_duration'] > DETECTION_DURATION)
    print(f"Sleep Count (durasi > {DETECTION_DURATION}s): {sleep_count}")

    # Update durasi dan cek notifikasi hanya untuk objek dengan durasi > DETECTION_DURATION
    for obj_id in list(tracked_objects.keys()):
        if obj_id in tracked_boxes:
            if tracked_objects[obj_id]['total_duration'] > DETECTION_DURATION and not tracked_objects[obj_id]['message_sent']:
                ...
                if image_url:
                    ...
                    print(f"Debug: timestamp={timestamp}, type={type(timestamp)}, image_url={image_url}, type={type(image_url)}, sleep_count={sleep_count}, obj_id={obj_id}")
                    save_to_firestore_detection(
                        date_str=date_str,
                        timestamp=timestamp,
                        image_url=image_url,
                        sleep_count=sleep_count,
                        obj_id=obj_id,
                        total_duration=tracked_objects[obj_id]['total_duration'],
    ...
    if cv2.waitKey(1) == ord('q'):
        # Program berhenti, tidak perlu menyimpan durasi
